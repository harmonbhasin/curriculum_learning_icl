...
def get_task_sampler(
    ...
    task_names_to_classes = {
        "hermite_regression": HermiteRegression,
...
class HermiteRegression(Task):
    def __init__(self, n_dims, batch_size, pool_dict=None, seeds=None, scale=1, degree=1, weight_type='standard'):
        """scale: a constant by which to scale the randomly sampled weights."""
        super(HermiteRegression, self).__init__(
    ...
    def evaluate(self, xs_b):
        ...
        alpha = 1
        if self.degree == 1:
            ys_b = self.scale * alpha * t
        elif self.degree == 2:
            alpha = 1 / pow(2, 1 / 2)
            ys_b = self.scale * alpha * (t + 1 / pow(factorial(2), 1 / 2) * (pow(t, 2) - 1))
        elif self.degree == 3:
            alpha = 1 / pow(3, 1 / 2)
            ys_b = self.scale * alpha * (t + 1 / pow(factorial(2), 1 / 2) *
                                         (pow(t, 2) - 1) + 1 / pow(factorial(3), 1 / 2) *
                                         (pow(t, 3) - 3 * t))
        elif self.degree == 4:
            alpha = 1 / pow(4, 1 / 2)
            ys_b = self.scale * alpha * (t + 
                                         1 / pow(factorial(2), 1 / 2) * (pow(t, 2) - 1) + 
                                         1 / pow(factorial(3), 1 / 2) * (pow(t, 3) - 3 * t) + 
                                         1 / pow(factorial(4), 1 / 2) * (pow(t, 4) -  6*pow(t,2) + 3))
...
class LinearRegression(Task):
    def __init__(self, n_dims, batch_size, pool_dict=None, seeds=None, scale=1, weight_type='standard'):
        """scale: a constant by which to scale the randomly sampled weights."""
